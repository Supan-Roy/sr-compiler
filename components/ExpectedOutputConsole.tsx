...
interface ExpectedOutputConsoleProps {
  output: string;
  onOutputChange: (newOutput: string) => void;
  fontSize: string;
}

export const ExpectedOutputConsole: React.FC<ExpectedOutputConsoleProps> = ({ output, onOutputChange, fontSize }) => {
  return (
    <div className="bg-slate-100 dark:bg-slate-800 rounded-lg shadow-inner flex-grow flex flex-col overflow-hidden border border-slate-200 dark:border-slate-700">
      <div className="bg-slate-200/50 dark:bg-slate-900/50 px-4 py-2 border-b border-slate-200 dark:border-slate-700 flex items-center gap-2">
        <Icon type="target" className="w-5 h-5 text-slate-500 dark:text-slate-400" />
        <h2 className="text-sm font-semibold text-slate-600 dark:text-slate-300">Expected Output</h2>
      </div>
      <textarea
        value={output}
